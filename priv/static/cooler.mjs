...
var BitArray = class {
  ...
  equals(other) {
    ...
    if (this.bitOffset === 0 && other.bitOffset === 0) {
      ...
      for (let i2 = 0; i2 < wholeByteCount; i2++) {
        const a = bitArrayByteAt(this.rawBuffer, this.bitOffset, i2);
...
function unequalBuffers(a, b) {
  return !(a instanceof BitArray) && a.buffer instanceof ArrayBuffer && a.BYTES_PER_ELEMENT && !(a.byteLength === b.byteLength && a.every((n, i2) => n === b[i2]));
}
...
function replace2(string5, pattern, substitute) {
  let _pipe = string5;
      ...
      if (prev.key !== next.key) {
        ...
        if ($ instanceof Fragment) {
          ...
          if ($1 instanceof Fragment) {
            ...
            let prev$1 = $;
            let old_remaining = old.tail;
            let change = replace3(node_index - moved_offset, next$1);
            let _block;
            let _pipe = events;
          ...
          if ($1 instanceof Element) {
            ...
            if (prev$1.namespace === next$1.namespace && prev$1.tag === next$1.tag) {
              ...
              let prev$2 = $;
              let old_remaining = old.tail;
              let change = replace3(node_index - moved_offset, next$2);
              let _block;
              let _pipe = events;
...
// build/dev/javascript/cooler/cooler_ffi.mjs
function addKeyListener(callback) {
  window.addEventListener("keydown", (event4) => {
    if (event4.key === " ") {
...
function copyToClipboard(text4) {
  navigator.clipboard.writeText(text4).catch((err) => {
...
// build/dev/javascript/cooler/cooler.mjs
var FILEPATH = "src/cooler.gleam";
...
var ColorBar = class extends CustomType {
  constructor(hex, locked) {
    super();
    this.hex = hex;
    this.locked = locked;
  }
...
var ToggleLock = class extends CustomType {
  constructor(index4) {
    super();
    this.index = index4;
  }
};
var RegenerateUnlocked = class extends CustomType {
};
var UpdateHex = class extends CustomType {
  constructor(index4, hex) {
    super();
    this.index = index4;
    this.hex = hex;
  }
};
var CopyHex = class extends CustomType {
  constructor(hex) {
    super();
    this.hex = hex;
  }
};
var AddColorAfter = class extends CustomType {
  constructor(index4) {
    super();
    this.index = index4;
  }
};
var RemoveColor = class extends CustomType {
  constructor(index4) {
    super();
    this.index = index4;
  }
};
var LoadFromUrl = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
function setup_key_listener(msg) {
  return from(
    (dispatch) => {
...
function copy_to_clipboard(text4) {
  return from((_) => {
    return copyToClipboard(text4);
  });
}
function setup_hash_listener() {
  return from(
    (dispatch) => {
      return onHashChange(
        (hash) => {
          return dispatch(new LoadFromUrl(hash));
        }
      );
    }
  );
}
function toggle_lock_at(palette, index4) {
  return index_map(
    palette,
    (bar, i2) => {
      let $ = i2 === index4;
      if ($) {
        return new ColorBar(bar.hex, !bar.locked);
      } else {
        return bar;
      }
    }
...
function update_hex_at(palette, index4, hex) {
  let _block;
  let $ = contains_string(hex, "#");
  if ($) {
    _block = hex;
  } else {
    _block = "#" + hex;
  }
  let updated_hex = _block;
  return index_map(
    palette,
    (bar, i2) => {
      let $1 = i2 === index4;
      if ($1) {
        return new ColorBar(updated_hex, bar.locked);
      } else {
        return bar;
      }
    }
...
function remove_color_at(palette, index4) {
  let _pipe = palette;
  let _pipe$1 = index_map(_pipe, (bar, i2) => {
    return [bar, i2];
  });
  let _pipe$2 = filter(_pipe$1, (pair) => {
    return pair[1] !== index4;
  });
  return map(_pipe$2, (pair) => {
    return pair[0];
  });
}
function encode_palette_to_url(palette) {
  let _pipe = palette;
  let _pipe$1 = map(
    _pipe,
    (bar) => {
      let hex_clean = replace2(bar.hex, "#", "");
      let _block;
      let $ = bar.locked;
      if ($) {
        _block = "1";
      ...
      let locked_flag = _block;
      return hex_clean + "-" + locked_flag;
    }
  );
  return join(_pipe$1, ",");
}
function update_url(palette) {
  return from(
    (_) => {
      let encoded = encode_palette_to_url(palette);
      return updateUrlHash(encoded);
    }
  );
}
function decode_url_to_palette(hash) {
  let _pipe = split3(hash, ",");
  return filter_map(
    _pipe,
    (segment) => {
      let $ = split3(segment, "-");
      if ($ instanceof Empty) {
        ...
        if ($1 instanceof Empty) {
          ...
          if ($2 instanceof Empty) {
            let hex = $.head;
            let locked_str = $1.head;
            let locked = locked_str === "1";
            return new Ok(new ColorBar("#" + hex, locked));
          } else {
            return new Error(void 0);
          }
        }
...
function hsl_to_hex(h, s, l) {
  let s_decimal = identity(s) / 100;
  let l_decimal = identity(l) / 100;
  let c = (1 - absolute_value(2 * l_decimal - 1)) * s_decimal;
  let h_prime = identity(h) / 60;
  let $ = modulo(h_prime, 2);
  let h_mod;
  if ($ instanceof Ok) {
    h_mod = $[0];
  } else {
  ...
  let x = c * (1 - absolute_value(h_mod - 1));
  let m = l_decimal - c / 2;
  let _block;
  let $2 = h_prime < 1;
  if ($2) {
  ...
  b_prime = $1[2];
  let _block$1;
  let _pipe = (r_prime + m) * 255;
  _block$1 = round(_pipe);
  let r = _block$1;
  let _block$2;
  let _pipe$1 = (g_prime + m) * 255;
  _block$2 = round(_pipe$1);
  let g = _block$2;
  let _block$3;
  let _pipe$2 = (b_prime + m) * 255;
  _block$3 = round(_pipe$2);
  let b = _block$3;
...
function generate_palette(existing) {
  return map(
    existing,
    (bar) => {
      let $ = bar.locked;
      if ($) {
        ...
        return new ColorBar(random_color(), false);
      }
    }
  );
}
function generate_initial_palette(count) {
...
function init(_) {
  let url_hash = getUrlHash();
  let _block;
  let $ = string_length(url_hash) > 0;
  if ($) {
    let decoded = decode_url_to_palette(url_hash);
    let $1 = is_empty2(decoded);
    if ($1) {
      _block = generate_initial_palette(5);
    } else {
      _block = decoded;
    }
  } else {
  ...
  let palette = _block;
  let key_listener = setup_key_listener(new RegenerateUnlocked());
  let hash_listener = setup_hash_listener();
  return [
    new Model(palette),
    batch(toList([key_listener, hash_listener]))
  ];
}
function regenerate_unlocked(palette) {
  return generate_palette(palette);
}
function get_text_color(hex) {
  let hex_clean = replace2(hex, "#", "");
  let $ = base_parse(slice(hex_clean, 0, 2), 16);
  let r;
  if ($ instanceof Ok) {
    r = $[0];
  } else {
  ...
  let $1 = base_parse(slice(hex_clean, 2, 2), 16);
  let g;
  if ($1 instanceof Ok) {
    g = $1[0];
  } else {
  ...
  let $2 = base_parse(slice(hex_clean, 4, 2), 16);
  let b;
  if ($2 instanceof Ok) {
    b = $2[0];
  } else {
  ...
  let luminance = 0.299 * identity(r) + 0.587 * identity(g) + 0.114 * identity(
    b
  );
  let $3 = luminance > 128;
  if ($3) {
...
function color_bar_view(bar, index4, total) {
  let text_color = get_text_color(bar.hex);
  let $ = from_string("^#([0-9a-fA-F]{6})$");
  if (!($ instanceof Ok)) {
    throw makeError(
      "let_assert",
      FILEPATH,
  ...
  let is_last = index4 === total - 1;
  return div(
    toList([class$("flex flex-row relative h-full w-full")]),
    toList([
      (() => {
        let $1 = index4 === 0;
        if ($1) {
            ...
            toList([
              button(
                toList([
                  ...
                  type_("button"),
                  on_click(new AddColorAfter(-1)),
                  attribute2("onmouseup", "this.blur()")
                ]),
                toList([
                  i(
                    toList([class$("iconoir-plus text-xl")]),
                    toList([])
                  )
                ])
              )
      ...
      div(
        toList([
          ...
          (() => {
            if (text_color instanceof Light) {
              return class$("text-white");
            } else {
              return class$("text-black");
            }
          })(),
          styles(toList([["background-color", bar.hex]]))
        ]),
        toList([
          div(
            toList([class$("font-heading text-2xl text-center font-bold")]),
            toList([
              input(
                toList([
                  class$(
                  ...
                  on_input(
                    (value2) => {
                      return new UpdateHex(index4, value2);
                    }
                  ),
                  attribute2("maxlength", "7")
                ])
              )
            ])
          ),
          button(
            toList([
              ...
              type_("button"),
              on_click(new ToggleLock(index4)),
              attribute2("onmouseup", "this.blur()")
            ]),
            toList([
              (() => {
          ...
          button(
            toList([
              ...
              type_("button"),
              on_click(new RemoveColor(index4)),
              attribute2("onmouseup", "this.blur()")
            ]),
            toList([
              i(
      ...
      div(
        toList([
          class$(
            (() => {
              if (is_last) {
                return "absolute content-center right-0 top-0 bottom-0 mr-0 z-2 group";
              } else {
                return "absolute content-center right-0 top-0 bottom-0 mr-[-20px] z-2 group";
              }
            })()
          )
        ]),
        toList([
          button(
            toList([
              ...
              type_("button"),
              on_click(new AddColorAfter(index4)),
              attribute2("onmouseup", "this.blur()")
            ]),
            toList([
              i(toList([class$("iconoir-plus text-xl")]), toList([]))
            ])
          )
        ])
      )
...
function view(model) {
  let palette_count = length(model.palette);
  return div(
    toList([]),
    toList([
      div(
        ...
        toList([
          ...
          div(
            toList([class$("flex flex-col mr-4")]),
            toList([
              p(
                toList([
                  class$(
                    "text-center text-lg text-gray-800 mt-2 font-body"
                  )
                ]),
                toList([text3("A simple color palette generator")])
              ),
              p(
                toList([
                  class$(
                    "text-center text-sm text-gray-800 mt-1 font-body"
                  )
                ]),
                toList([
                  text3("Press "),
                  ...
                  text3(" to regenerate unlocked colors")
                ])
              )
            ])
          )
      ...
      div(
        toList([class$("flex flex-row absolute h-full w-full")]),
        index_map(
          model.palette,
          (bar, index4) => {
            return color_bar_view(bar, index4, palette_count);
          }
        )
      )
    ])
...
function average_colors(hex1, hex2) {
  let clean1 = replace2(hex1, "#", "");
  let clean2 = replace2(hex2, "#", "");
  let $ = base_parse(slice(clean1, 0, 2), 16);
  let r1;
  if ($ instanceof Ok) {
    r1 = $[0];
  } else {
  ...
  let $1 = base_parse(slice(clean1, 2, 2), 16);
  let g1;
  if ($1 instanceof Ok) {
    g1 = $1[0];
  } else {
  ...
  let $2 = base_parse(slice(clean1, 4, 2), 16);
  let b1;
  if ($2 instanceof Ok) {
    b1 = $2[0];
  } else {
  ...
  let $3 = base_parse(slice(clean2, 0, 2), 16);
  let r2;
  if ($3 instanceof Ok) {
    r2 = $3[0];
  } else {
  ...
  let $4 = base_parse(slice(clean2, 2, 2), 16);
  let g2;
  if ($4 instanceof Ok) {
    g2 = $4[0];
  } else {
  ...
  let $5 = base_parse(slice(clean2, 4, 2), 16);
  let b2;
  if ($5 instanceof Ok) {
    b2 = $5[0];
  } else {
  ...
  let r = globalThis.Math.trunc((r1 + r2) / 2);
  let g = globalThis.Math.trunc((g1 + g2) / 2);
  let b = globalThis.Math.trunc((b1 + b2) / 2);
  return "#" + int_to_hex(r) + int_to_hex(g) + int_to_hex(b);
}
function add_color_after(palette, index4) {
  if (index4 === -1) {
    let $ = first(palette);
    if ($ instanceof Ok) {
      return prepend(new ColorBar(random_color(), false), palette);
    } else {
      return palette;
    }
  } else {
    let before = take(palette, index4 + 1);
    let after = drop(palette, index4 + 1);
    let _block;
    let $ = last(before);
    let $1 = first(after);
    if ($1 instanceof Ok) {
      if ($ instanceof Ok) {
        let right = $1[0];
        let left = $[0];
        _block = average_colors(left.hex, right.hex);
      } else {
        _block = random_color();
      }
    ...
    return append(
      before,
      prepend(new ColorBar(new_hex, false), after)
    );
  }
}
function update2(model, msg) {
  if (msg instanceof ToggleLock) {
    let index4 = msg.index;
    let new_palette = toggle_lock_at(model.palette, index4);
    return [new Model(new_palette), update_url(new_palette)];
  } else if (msg instanceof RegenerateUnlocked) {
    let new_palette = regenerate_unlocked(model.palette);
    return [new Model(new_palette), update_url(new_palette)];
  } else if (msg instanceof UpdateHex) {
    let index4 = msg.index;
    let hex = msg.hex;
    let new_palette = update_hex_at(model.palette, index4, hex);
    return [new Model(new_palette), update_url(new_palette)];
  } else if (msg instanceof CopyHex) {
    let hex = msg.hex;
    return [model, copy_to_clipboard(hex)];
  } else if (msg instanceof AddColorAfter) {
    let index4 = msg.index;
    let new_palette = add_color_after(model.palette, index4);
    return [new Model(new_palette), update_url(new_palette)];
  } else if (msg instanceof RemoveColor) {
    let index4 = msg.index;
    let $ = length(model.palette) > 2;
    if ($) {
      let new_palette = remove_color_at(model.palette, index4);
      return [new Model(new_palette), update_url(new_palette)];
    } else {
      return [model, none()];
    }
    ...
    let $ = is_empty2(decoded);
    if ($) {
      return [model, none()];
    } else {
      return [new Model(decoded), none()];
    }
  }
}
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      "main",
      "Pattern match failed, no pattern matched the value.",
      { value: $, start: 504, end: 553, pattern_start: 515, pattern_end: 520 }
    );
  }
  return void 0;
}
